<template>
  <div>
    <h2>this is the kickback Page</h2>
    <div v-if="loading">Loading React component...</div>
    <div v-else-if="error">Error: {{ error }}</div>
    <div v-else ref="kickbackContainer"></div>
  </div>
</template>
...
const kickbackContainer = ref<HTMLDivElement | null>(null)
...
onMounted(async () => {
  try {
    ...
    if (kickbackContainer.value) {
      reactRoot = ReactDOM.createRoot(kickbackContainer.value)
